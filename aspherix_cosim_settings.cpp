...
using SettingValueType = std::variant<
    bool, int, std::size_t, double, std::string, std::vector<double>,
...
public:
  ...
  bool operator==(const CoSimSettingsImpl &other) const {
    ...
    return std::equal(items_.begin(), items_.end(), other.items_.begin(),
                      item_comparator);
  }
  ...
  std::size_t
  length(const SyncDirection &direction = SyncDirection::kUndefined) const {
    std::size_t length = 0;

    for (const auto &item : items_) {
      if (item.direction == direction ||
          direction == SyncDirection::kUndefined) {
        std::visit(
            [&length](auto &&value) {
              ...
              if constexpr (IsStdArrayDouble<T>::value) {
                ...
                if constexpr (std::is_same_v<D, std::string>) {
                  ...
                  total_length +=
                      std::accumulate(value.begin(), value.end(), 0U,
  ...
  void
  fromByteVector(const std::vector<char> &byte_array, std::size_t offset = 0,
            ...
            [&offset, byte_array, this](auto &&value) {
              ...
              if constexpr (IsStdArrayDouble<T>::value) {
                ...
                const auto vector_length =
                    extract<std::size_t>(byte_array, offset);
  ...
  std::vector<char> toByteVector(
      const SyncDirection &direction = SyncDirection::kUndefined) const {
            ...
            [&result, this](auto &&value) {
              ...
              if constexpr (IsStdArrayDouble<T>::value) {
                for (const auto &array_value : value) {
                  const auto *bytes =
                      reinterpret_cast<const char *>(&array_value);
                  std::copy(bytes, bytes + sizeof(double),
                            std::back_inserter(result));
                }
              } else if constexpr (std::is_same_v<T, std::string>) {
                const std::size_t str_length = value.size();
                const auto *bytes_len =
                    reinterpret_cast<const char *>(&str_length);
                std::copy(bytes_len, bytes_len + sizeof(std::size_t),
                          std::back_inserter(result));
                const auto *bytes =
                    value.c_str(); // c_str is '\0' terminated, so +1
                std::copy(bytes, bytes + str_length + 1,
                          std::back_inserter(result));
              } else if constexpr (IsStdVector<T>::value) {
                ...
                std::copy(bytes, bytes + sizeof(data),
...
void CoSimSettings::clearSettings() { pimpl_->clearSettings(); }

template <typename T>
void CoSimSettings::addSetting(const std::string &name, T value,
                               SyncDirection direction) {
  pimpl_->addSetting(name, value, direction);
}

template <typename T>
void CoSimSettings::setSetting(const std::string &name, T new_value) {
  pimpl_->setSetting(name, new_value);
}

template <typename T>
auto CoSimSettings::getSetting(const std::string &name) const -> T {
